...
const GravamenesPage: React.FC = () => {
    ...
    const filtrarGravamenes = () => {
        ...
        if (estadoGravamen && estadoGravamen !== "true") {
            gravamenesFiltrados = gravamenesFiltrados.filter((gravamen) => gravamen.estadoGravamen === (estadoGravamen === "true"));
        }

        if (nombreGravamen) {
            gravamenesFiltrados = gravamenesFiltrados.filter((gravamen) => {
                return gravamen.nombreGravamen.toLowerCase().includes(nombreGravamen.toLowerCase());
            });
        }
    ...
    return (
        <ContenedorPrincipal>
            ...
            <div className="grid gap-4 md:grid-cols-3">
                {gravamenesFiltrados.map((gravamen) => (
            ...
            <Modal isOpen={modalInfo} setIsOpen={() => setModalInfo(false)}>
                {gravamenSeleccionado && <MostrarInfoGravamen gravamen={gravamenSeleccionado} />}
            </Modal>

            <Modal isOpen={modalRegistrar} setIsOpen={() => setModalRegistrar(false)}>
                <RegistrarGravamen obtenerGravamenes={obtenerGravamenes} setModalRegistrar={setModalRegistrar} />
            </Modal>

            <Modal isOpen={modalActualizar} setIsOpen={() => setModalActualizar(false)}>
                <RegistrarGravamen idGravamen={gravamenSeleccionado?.idGravamen} obtenerGravamenes={obtenerGravamenes} setModalActualizar={setModalActualizar} />
            </Modal>
        </ContenedorPrincipal>
    );
};
