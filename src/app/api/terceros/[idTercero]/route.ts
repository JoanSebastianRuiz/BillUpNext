...
export const PUT = async (request: NextRequest, context: { params: Promise<{ idTercero: string }> }) => {
    const { searchParams } = request.nextUrl;
    const tipo = searchParams.get("tipo");
    const { idTercero } = await context.params;
...
export const GET = async (request: NextRequest, context: { params: Promise<{ idTercero: string }> }) => {
    const { searchParams } = request.nextUrl;
    const tipo = searchParams.get("tipo");
    const { idTercero } = await context.params;
